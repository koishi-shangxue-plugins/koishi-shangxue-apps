...
export function apply(ctx: Context, config: Config) {
  ...
  if (config.mode === 'webhook') {
    ...
    // 使用 Koishi 的 HTTP 服务
    ctx.inject(['server','router'], (serverCtx) => {
      serverCtx.on('ready', () => {
        const router = (serverCtx as any).server || (serverCtx as any).router
        if (!router) {
          logger.error('无法获取 HTTP 路由器，Webhook 模式需要 server 服务')
          return
        }

        router.post(webhookPath, async (koaCtx: any) => {
