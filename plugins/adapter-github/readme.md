...
2.  点击 **"Generate new token"**，然后从弹出的菜单中选择 **"Generate new token (classic)"**。
...
- **`owner`**: 填写你想要监听的仓库的所有者名称（例如 `koishi-shangxue-plugins`）。
- **`repo`**: 填写你想要监听的仓库名称（例如 `koishi-shangxue-apps`）。
