...
declare module '@koishijs/plugin-console' {
  ...
  interface Events {
    ...
    'webdialogue/create'(dialogue: Omit<Dialogue, 'id'> & Partial<Pick<Dialogue, 'id'>>): Promise<{ success: boolean; message?: string }>
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.on("ready", async () => {
    ...
    const normalizeDialogueCreate = (input: Omit<Dialogue, 'id'> & Partial<Pick<Dialogue, 'id'>>): Omit<Dialogue, 'id'> & Partial<Pick<Dialogue, 'id'>> => {

      const question = (input.question || '').trim()

      const answer = (input.answer || '').trim()

      const filterGroups = Array.isArray(input.filterGroups) ? input.filterGroups.slice(0, 1) : input.filterGroups

      return { ...input, question, answer, filterGroups }

    }

    const normalizeDialogueUpdate = (input: Dialogue): Dialogue => {
    ...
    ctx.console.addListener('webdialogue/create', async (dialogue) => {
      ...
      await ctx.database.create('webdialogue', normalizeDialogueCreate(dialogue))
    ...
    ctx.console.addListener('webdialogue/update', async (dialogue) => {
      ...
      await ctx.database.upsert('webdialogue', [normalizeDialogueUpdate(dialogue)])
